...
# the agent, enrolls it against the panel with a single-use join token minted
# there, and leaves a systemd service reporting in every 30 seconds.
#
...
SR-UI node agent installer
  ...
  --core PATH      xray binary, used only so the panel can show its version
...
do_install() {
  ...
  printf '\n'
  printf 'This agent reports in, stores the assignment the panel gives it, and\n'
  printf 'runs %s/apply.sh when one arrives. Until that hook exists the\n' "$DIR"
  printf 'panel will show this node as out of sync, which is accurate: nothing\n'
  printf 'on this machine is serving the assignment yet.\n'
}
